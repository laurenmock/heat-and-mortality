...
### Script 1 ### 
# Input: Raw Data 
# (la, la.mortality, chic, chic.mortality, ny, ny.mortality, pitt, pitt.mortality, seat, seat.mortality)
...
# path for writing processed data (use "" to write to general path)
processed_data_path <- "data/processed/"

# source functions
source("R_code/excessDeaths_functions.R")
...
if (exists("relevant_variables") && !is.null(relevant_variables)){
  ...
  for (i in 1:length(relevant_variables)){
    ...
    if (nb_missing>0){
      ...
      cat("\n >>>>>>>>> WARNING: MISSING", nb_missing_percent, "%", relevant_variables[i],"values <<<<<<<<<\n")
    }
  }
...
raw_data %>%
    filter(is.na(pm25)) %>%
    ggplot() +
    geom_histogram(aes(isoyear(date))) +
    facet_grid(~city)
...
# For each covariate, form new covariate defined as the value of that covariate on (day - 1), (day - 2), ...
# up to (day - nb_lag)
...
processed_data$dow_cats <-  case_when(
  ...
  processed_data$dow == "Tuesday" | processed_data$dow == "Wednesday" | processed_data$dow == "Thursday" ~ "Middle")
...
#g <- arrangeGrob(corr_plots[[1]], corr_plots[[2]], corr_plots[[3]], corr_plots[[4]], corr_plots[[5]], nrow = 5)
...
#save(treatment_defined, file = paste0(processed_data_path, "all_processed_data.Rda"))
